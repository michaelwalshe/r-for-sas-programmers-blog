...
2.  Use a data step to edit our dataset, first filtering to just observations where gender is equal to `"M"` and only keeping the variables `height`, `salary`, and `weight` on the input.
...
result <- 
  read_csv("data/demog.csv") |>             # <1>
  filter(gender == "M") |>                  # <2>
  select(height, weight, salary) |>         # <3>
  mutate(height_m = (height * 2.54) / 100,  # <4> 
         bmi = weight / (height_m **2)) |> 
  cor.test(~ bmi + salary,                  # <5>
           data = _,
           na.action = "na.omit") 
```
1.  Read in our data using `read_csv` from [`{readr}`](https://readr.tidyverse.org/). This is similar to the base R function `read.csv`, but is faster and with better defaults. We then "pipe" the output of this function in to the next line using `|>`, this is a base R feature to make code calling several functions in a row more readable. Otherwise, this would look something like: `mutate(filter(select(read_csv("file"), ...), ...), ...)`. Much nicer with pipes!
2.  Filter the data using the `filter` function from [`{dplyr}`](https://dplyr.tidyverse.org/). This is similar to indexing with square brackets, but is more readable and user-friendly.
3.  Choose just a couple of columns using the `select` function from `{dplyr}`. This is equivalent to the second half of our base R indexing.
4.  Add new columns using the `mutate` function from `{dplyr}`. This is a very powerful function, with a lot more functionality via additional `{dplyr}` functions.
5.  Again use the `cor.test` function, though note that to specify the `data` argument we use an underscore to indicate that the pipe should insert the previous result at this place in the function.
